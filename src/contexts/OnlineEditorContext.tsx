...
type OnlineEditorAction =
  ...
  | { type: 'SET_FILTERS'; payload: { grade?: string; gender?: string } }
  | { type: 'RESET_STATE' };

// Reducer函数
function onlineEditorReducer(
  state: OnlineEditorState,
  action: OnlineEditorAction
): OnlineEditorState {
  switch (action.type) {
    case 'SET_LOADING':
    ...
    case 'SET_FILTERS':
      return {
        ...state,
        filterGrade: action.payload.grade || state.filterGrade,
        filterGender: action.payload.gender || state.filterGender,
      };
    case 'RESET_STATE':
      return initialState;
    default:
...
// Provider组件
interface OnlineEditorProviderProps {
  children: ReactNode;
}

export const OnlineEditorProvider: React.FC<OnlineEditorProviderProps> = ({ children }) => {
